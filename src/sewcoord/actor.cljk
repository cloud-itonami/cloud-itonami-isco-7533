(ns sewcoord.actor
  "SewCoordActor — the ISCO-08 7533 sewing/embroidery workshop
  ...
  resume after human sign-off. ISCO-08 7533 (Sewing, Embroidery and
  Related Workers) operates sewing/embroidery machines — a standard
  workshop hazard profile (needle-puncture injury, moving-machine-
  part injury) — so this actor's scope stays deliberately narrow to
  workshop scheduling/logistics coordination only, never sewing/
  embroidery execution or safety-clearance authority. Modeled closely
  ...
  The unconditional invariant: the Sewing Coordination Advisor can
  never directly commit an operating record the SewCoordGovernor
  refuses, and can never make this actor finalize a sewing/embroidery-
  execution decision (e.g. deciding a sewing or embroidery job is
  finished) or a workshop-safety-clearance decision (e.g. declaring a
  workshop or item safety-cleared), nor override a shop safety
  officer's judgment — every commit-record! call is gated behind
  `:decide`, and that decision class is a permanent hard block
  regardless of disposition path. This actor coordinates WORKSHOP
  SCHEDULING/LOGISTICS ONLY — it never performs sewing or embroidery
...
(defn build-graph
  "Build a compiled SewCoordActor graph. `store` implements
  `sewcoord.store/Store`. `advisor` implements
  `sewcoord.advisor/Advisor` (defaults to `mock-advisor`).
